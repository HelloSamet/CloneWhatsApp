...
//  CloneWhatsApp
//
//  Created by Abdulsamet Göçmen on 1.01.2025.
//
    ...
    static func paginateUsers(lastCursor: String?, pageSize: UInt) async throws -> UserNode {
        ...
        if lastCursor == nil {
            ...
            if users.count == mainSnapshot.childrenCount {
                let userNode = UserNode(users: users, currenCursor: first.key)
                return userNode
            }
            ...
            if users.count == mainSnapshot.childrenCount {
                ...
                let userNode = UserNode(users: filteredUsers, currenCursor: first.key)
                return userNode
            }
...
struct UserNode {
    var users: [UserItem]
    var currenCursor: String?
    static let emptyNode = UserNode(users: [], currenCursor: nil)
}
